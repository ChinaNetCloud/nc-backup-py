...
class AWSS3(Storage):
    ...
    def upload_content(self, mypath_to_dir, bucket, client_host_name, upload_command='aws s3 cp'):
        ...
        files_to_upload = [mypath_to_dir + f for f in listdir(mypath_to_dir) if isfile(join(mypath_to_dir, f))]
        sys.path.append(self.__home_path)
        from execution.subprocess_execution import SubprocessExecution
        for file_to_upload in files_to_upload:
            aws_command = upload_command + ' '+ file_to_upload + ' s3://'+ bucket + '/' + client_host_name + '/'
